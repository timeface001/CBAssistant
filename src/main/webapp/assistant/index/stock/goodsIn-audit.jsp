...
<div class="pd-20">
    <div class="Huiform">
        <form id="auditForm" class="form form-horizontal">
            <input type="hidden" value="<%=id%>" name="id">
            <input type="hidden" value="<%=pSku%>" name="pSku">
            <input type="hidden" value="<%=totalPrice%>" name="totalPrice">
            <input type="hidden" value="<%=pInNum%>" name="pInNum">
            <input type="hidden" value="2" name="pState">
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>实际入库数：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="<%=pInNum%>" placeholder="" id="actualPInNum"
                           name="actualPInNum">
                </div>
            </div>
            <div class="row cl">
...
<script type="text/javascript">
    ...
    function ajaxSubmit() {
        var actualPInNum = $("#actualPInNum").val();
        if(actualPInNum > pInNum){
            layer.msg('实际入库量不能大于新建入库量', {icon: 2, time: 2000});
            return false;
        }
        layer.load();
        $.ajax({
            type: 'POST',
            dataType: 'json',
            data: {"data": JSON.stringify(getFormJson("#auditForm"))},
            url: "<%=request.getContextPath()%>/stock/auditGoodsIn",
