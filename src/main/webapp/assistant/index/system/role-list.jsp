...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray" id="count-div"><span class="l"><a class="btn btn-primary radius" href="javascript:;"
                                 onclick="addRole('添加角色','<%=request.getContextPath()%>/assistant/index/system/role-add.jsp','800')"><i
            class="Hui-iconfont">
        &#xe600;</i> 添加角色</a> </span></div>
    <table id="roleTable" class="table table-border table-bordered table-hover table-bg">
        <thead>
        <tr class="text-c">
...
<script type="text/javascript">
                    ...
                    $('#roleTable').dataTable({
                        ...
                        "columnDefs": [
                            ...
                            },
                            {
                                ...
                                "render": function (data, type, full) {
                                    if (data == 1) {
                                        return "<a style='text-decoration:none' title='停用' onClick=\"stopRole('" + full.ROLE_ID + "')\"'><i class='Hui-iconfont'>&#xe631;</i></a>" +
                                                "<a style='text-decoration:none' title='编辑' onClick=\"editRole('" + full.ROLE_ID + "')\"'><i class='Hui-iconfont'>&#xe6df;</i></a>";
                                    } else {
                                        return "<a style='text-decoration:none' title='启用' onClick=\"startRole('" + full.ROLE_ID + "')\"'><i class='Hui-iconfont'>&#xe615;</i></a>" +
                                                "<a style='text-decoration:none' title='编辑'  onClick=\"editRole('" + full.ROLE_ID + "')\"'><i class='Hui-iconfont'>&#xe6df;</i></a>";
                                    }
                                }
                            }
    ...
    /*角色-添加*/
    function addRole(title, url, w) {
        layer_show(title, url, w);
    }
    /*角色-编辑*/
    function editRole(id) {
        layer_show('编辑角色', '<%=request.getContextPath()%>/assistant/index/system/role-add.jsp?id=' + id, '800');
    }
    /*角色-删除*/
    function deleteRole(id) {
        layer.confirm('角色删除须谨慎，确认要删除吗？', function (index) {
            $.ajax({
                type: 'POST',
                url: '<%=request.getContextPath()%>/system/deleteRole',
                dataType: 'json',
                data: {"id": id},
                success: function (data) {
                    if (data.code == 0) {
    ...
    function stopRole(id) {
        layer.confirm('确认要停用吗？', function (index) {
            $.ajax({
                type: 'POST',
                url: '<%=request.getContextPath()%>/system/updateRoleState',
                dataType: 'json',
                data: {"id": id, "state": "0"},
                success: function (data) {
                    if (data.code == 0) {
