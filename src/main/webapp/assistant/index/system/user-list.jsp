...
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray" id="count-div"><span class="l"> <a href="javascript:;" onclick="deleteUser()"
                                                                         class="btn btn-danger radius"><i
            class="Hui-iconfont">
        &#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" href="javascript:;"
                                 onclick="addUser('添加用户','<%=request.getContextPath()%>/assistant/index/system/user-add.jsp','800')"><i
            class="Hui-iconfont">
        &#xe600;</i> 添加用户</a> </span></div>
    <table id="userTable" class="table table-border table-bordered table-hover table-bg">
        <thead>
        <tr class="text-c">
...
<script type="text/javascript">
                    ...
                    $('#userTable').dataTable({
                        ...
                        "columnDefs": [
                            ...
                            },
                            {
                                ...
                                "render": function (data, type, full) {
                                    if (data == 1) {
                                        return "<a style='text-decoration:none' title='停用' onClick=\"stopUser('" + full.USER_ID + "')\"'><i class='Hui-iconfont'>&#xe631;</i></a>" +
                                                "<a style='text-decoration:none' title='编辑' onClick=\"editUser('" + full.USER_ID + "')\"'><i class='Hui-iconfont'>&#xe6df;</i></a>" +
                                                "<a style='text-decoration:none' title='删除' onClick=\"deleteUser('" + full.USER_ID + "')\"'><i class='Hui-iconfont'>&#xe6e2;</i></a>";
                                    } else {
                                        return "<a style='text-decoration:none' title='启用' onClick=\"startUser('" + full.USER_ID + "')\"'><i class='Hui-iconfont'>&#xe615;</i></a>" +
                                                "<a style='text-decoration:none' title='编辑'  onClick=\"editUser('" + full.USER_ID + "')\"'><i class='Hui-iconfont'>&#xe6df;</i></a>" +
                                                "<a style='text-decoration:none' title='删除'  onClick=\"deleteUser('" + full.USER_ID + "')\"'><i class='Hui-iconfont'>&#xe6e2;</i></a>";
                                    }
                                }
    ...
    /*用户-添加*/
    function addUser(title, url, w) {
        layer_show(title, url, w);
    }
    /*用户-编辑*/
    function editUser(id) {
        layer_show('编辑用户', '<%=request.getContextPath()%>/assistant/index/system/user-add.jsp?id=' + id, '800');
    }
    /*用户-删除*/
    function deleteUser(id) {
        layer.confirm('用户删除须谨慎，确认要删除吗？', function (index) {
            $.ajax({
                type: 'POST',
                url: '<%=request.getContextPath()%>/system/deleteUser',
                dataType: 'json',
                data: {"id": id},
                success: function (data) {
                    if (data.code == 0) {
    ...
    function stopUser(id) {
        layer.confirm('确认要停用吗？', function (index) {
            $.ajax({
                type: 'POST',
                url: '<%=request.getContextPath()%>/system/updateUserState',
                dataType: 'json',
                data: {"id": id, "state": "0"},
                success: function (data) {
                    if (data.code == 0) {
